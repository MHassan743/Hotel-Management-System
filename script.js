...
function setUserRole(role) {
    ...
    updateUIForUserRole();
    showToast(`User role set to: ${role.toUpperCase()}`, 'info');
}
...
function updateUIForUserRole() {
    const actionButtons = document.querySelectorAll('.edit-booking, .delete-booking, .edit-guest, .delete-guest, .edit-room, .delete-room, .change-status, .edit-service, .toggle-service, .complete-request, .cancel-request, .toggle-vip, .check-in, .book-room-btn');
    ...
    if (userRoleElement) {
        userRoleElement.textContent = `${currentUser.name} (${currentUser.role.toUpperCase()})`;
    }
}
...
function setupEventListeners() {
    ...
    if (switchToAdminBtn) {
        switchToAdminBtn.addEventListener('click', () => {
            setUserRole('admin');
            location.reload(); // Reload to update all rendered content
        });
    }
    ...
    if (onlineBookingForm) {
        onlineBookingForm.addEventListener('submit', (e) => {
            ...
            if (!isAdmin()) {
                showToast('Only administrators can make online bookings', 'error');
                return;
            }
...
// Render functions with role-based access control
...
function renderOnlineRooms() {
    ...
    availableRooms.forEach((room, index) => {
        ...
        roomCard.innerHTML = `
            ...
            ` : '<button class="btn btn-secondary" disabled>Booking Disabled</button>'}
        `;
...
function processOnlineBooking(bookingData) {
    if (!isAdmin()) {
        showToast('Only administrators can process online bookings', 'error');
        return;
    }
    const newBooking = {
        id: `BK${String(sampleBookings.length + 1).padStart(3, '0')}`,
...
function selectRoomForBooking(roomNumber) {
    if (!isAdmin()) {
        showToast('Only administrators can select rooms for booking', 'error');
        return;
    }
...
function toggleOnlineBooking(enabled) {
    if (!isAdmin()) {
        showToast('Only administrators can toggle online booking', 'error');
        document.getElementById('onlineBookings').checked = !enabled;
        return;
    }
